...
export default function GradientWave() {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (let x = 0; x < canvas.width; x += 20) {
        const y = canvas.height / 2 + Math.sin(x * 0.01 + time) * 30 + waveFactorX * Math.sin(x * 0.02)
        ctx.lineTo(x, y)
      }
  ...
  return <canvas ref={canvasRef} className="fixed top-0 left-0 w-full h-full pointer-events-none z-0 opacity-10" />
}
